...
llmgate даёт **OpenAI-совместимый API**, поэтому используй как угодно. На вход — обычный POST на `/v1/chat/completions` (как у OpenAI). Что внутри — куча free-провайдеров с автоматическим фоллбэком. Главные сценарии:

- 🌐 **Свой сайт / приложение** — где раньше шёл в OpenAI SDK (Python, JS, любой язык), просто меняешь `base_url` на `http://your-llmgate:8765/v1` и `api_key` на свой токен. Остальной код не трогается.
- 💬 **Чат прямо в браузере без кода** — встроенный dashboard на `/dashboard` со статистикой и тестовым чатом по любой цепочке.
- 🛠️ **Backend для AI-кодеров** — Claude Code, Cursor, OpenCode, Aider, Continue, Cline, Zed AI. В настройках клиента: `base_url = http://your-llmgate:8765/v1`. Платный API заменяется на твой бесплатный.
- 🤖 **Бот для Telegram / Discord / Slack** — любая библиотека-обёртка над OpenAI работает как есть.
- 📊 **Скрипты автоматизации** — суммаризация писем, перевод документов, классификация тикетов, генерация постов. curl или питоновский скрипт в две строки.
- 🎨 **Генерация картинок** — `model: "image_gen"` (FLUX, Kandinsky, SDXL, бесплатно).
- 🔊 **Озвучка текста** — `model: "tts"` (Edge TTS, безлимит).
- 🔍 **Веб-поиск** — `model: "web"` (Gemini google_search + OpenRouter `:online`).
- 🧠 **Reasoning-агенты** — встроенные `moa` / `sc` / `debate` / `deep_search` без LangChain/LangGraph.

Короче: фантазия твоя, у тебя теперь есть бесплатный OpenAI-совместимый API.

## Установка — два пути

Выбирай по уверенности в командной строке. Оба пути закончатся одинаково — рабочий llmgate либо локально, либо на сервере.

### Путь 1: попросить AI сделать всё за тебя 🪄

Самый простой. Нужен один из «терминальных AI» — это CLI-инструмент с ИИ-агентом, который сам читает твои файлы и выполняет команды:

- **[Claude Code](https://claude.com/claude-code)** (Anthropic) — рекомендуется, лучше всех справляется с DevOps-задачами.
- **[OpenCode](https://github.com/sst/opencode)** — open-source альтернатива.
- **[Aider](https://aider.chat)** — старый и надёжный.

⚠️ **Не используй веб-чаты** (claude.ai, chatgpt.com) — они не имеют доступа к терминалу и будут просто давать советы. Нужна именно CLI-программа на твоей машине.

#### Локально (на своём компе)

Открой терминал в любой папке, запусти `claude` (или `opencode` / `aider`) и скажи примерно так:

> Склонируй репозиторий `https://github.com/basaltcode/neurogate` в папку `~/llmgate`. Установи `uv` если его нет, выполни `uv sync`, скопируй `.env.example` в `.env` и `config.yaml.example` в `config.yaml`. Я добавлю `GROQ_API_KEY` в `.env` сам — подскажи когда. После этого запусти сервер через `uv run llmgate` и открой dashboard в браузере.

Когда AI попросит ключ — открой [console.groq.com](https://console.groq.com) → войди → *API Keys* → *Create new* → скопируй и впиши в `.env` (или отдай AI). Дальше всё пойдёт само.
...
При покупке выбирай Ubuntu 24.04, добавь свой SSH-ключ (или используй временный пароль).

**2. Дай AI задачу** (в локальном Claude Code / OpenCode на твоём компе):

> У меня VPS `<IP>`, root-доступ по SSH с ключом `~/.ssh/id_ed25519` (или паролем `<пароль>`). Разверни на нём `https://github.com/basaltcode/neurogate` в `/opt/llmgate`. Поставь `uv`, выполни `uv sync`, скопируй конфиги из `.example`, помоги мне вписать `GROQ_API_KEY` в `.env`. Сгенерируй случайный `NEUROGATE_API_TOKEN`, поставь `LLMGATE_HOST=0.0.0.0`. Создай systemd unit `llmgate.service`, открой 22 и 8765 в ufw, запусти. После запуска проверь `http://<IP>:8765/health` и покажи мне URL дашборда.

AI всё сделает. Если что-то не так — он сам спросит или починит.

⚠️ **Важно: не отдавай пароли в веб-чаты AI** (claude.ai, chatgpt.com). Только в **локальный CLI** на твоей машине — он не отправляет твои файлы и пароли никуда лишнего, разговор идёт только между тобой и провайдером AI напрямую.

### Путь 2: руками (если хочешь понять что происходит)
...
```bash
git clone https://github.com/basaltcode/neurogate.git llmgate
cd llmgate
uv sync
```
...
Открой `.env` в любом редакторе и впиши хотя бы `GROQ_API_KEY=gsk_...` — взять на [console.groq.com](https://console.groq.com) → *API Keys* → *Create new*. Чем больше ключей — тем стабильнее (полный гайд: [docs/providers-setup.md](docs/providers-setup.md)).
...
```bash
git clone https://github.com/basaltcode/neurogate.git /opt/llmgate
cd /opt/llmgate
uv sync
cp config.yaml.example config.yaml
cp .env.example .env

# Сгенерируй случайный API-токен для защиты твоего эндпоинта
echo "NEUROGATE_API_TOKEN=llmgate_$(openssl rand -hex 24)" >> .env

# Поставь сервер слушать все интерфейсы (а не только localhost)
echo "LLMGATE_HOST=0.0.0.0" >> .env

# Открой .env и впиши провайдерские ключи (минимум GROQ_API_KEY)
nano .env

# Защити .env от чтения другими пользователями
chmod 600 .env
```
...
```bash
cat > /etc/systemd/system/llmgate.service <<'EOF'
[Unit]
Description=llmgate
...
ExecStart=/opt/llmgate/.venv/bin/llmgate
Restart=on-failure
RestartSec=5
...
systemctl enable --now llmgate
systemctl status llmgate     # должен быть active (running)
journalctl -u llmgate -n 30  # покажет стартап-баннер
```
...
```bash
cd /opt/llmgate && git pull && uv sync && systemctl restart llmgate
```

---

Не получилось — открой [issue](../../issues/new?template=bug_report.md) и приложи `journalctl -u llmgate -n 100`.
...
Body: `{prompt: str, duration?: 1-30s, model?: "sfx" | "hfspace:..."}`. Возвращает raw audio (`audio/wav` по умолчанию). Headers ответа: `X-Llmgate-Provider`, `X-Llmgate-Chain`, `X-Llmgate-Duration`. Для синтеза голоса используйте `POST /v1/audio/speech` (цепочка `tts`); для распознавания речи — `POST /v1/audio/transcriptions` (цепочка `audio`).
...
- **`/metrics`** — Prometheus-endpoint (без auth, стандарт для скраперов). Метрики:
  - `llmgate_requests_total{provider,outcome}` — счётчик запросов по исходам: `success` / `rate_limit` / `server_error` / `empty` / `timeout` / `decommissioned` / `rate_capped` / `other`
  - `llmgate_request_duration_seconds{provider,outcome}` — гистограмма латентности (Grafana считает p50/p95 через `histogram_quantile()`)
...
## Когда брать llmgate, когда не брать
...
- агент с длинной цепочкой шагов (>5 tool calls подряд) — стек llmgate начинает разваливаться;
