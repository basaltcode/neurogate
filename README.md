...
- 🌐 **Свой сайт / приложение** — где раньше шёл в OpenAI SDK (Python, JS, любой язык), просто меняешь `base_url` на `http://your-neurogate:8765/v1` и `api_key` на свой токен. Остальной код не трогается.
- 💬 **Чат прямо в браузере без кода** — встроенный dashboard на `/dashboard` со статистикой и тестовым чатом по любой цепочке.
- 🛠️ **Backend для AI-кодеров** — Claude Code, Cursor, OpenCode, Aider, Continue, Cline, Zed AI. В настройках клиента: `base_url = http://your-neurogate:8765/v1`. Платный API заменяется на твой бесплатный.
...
> Склонируй репозиторий `https://github.com/basaltcode/neurogate` в папку `~/neurogate`. Установи `uv` если его нет, выполни `uv sync`, скопируй `.env.example` в `.env` и `config.yaml.example` в `config.yaml`. Я добавлю `GROQ_API_KEY` в `.env` сам — подскажи когда. После этого запусти сервер через `uv run neurogate` и открой dashboard в браузере.
...
> У меня VPS `<IP>`, root-доступ по SSH с ключом `~/.ssh/id_ed25519` (или паролем `<пароль>`). Разверни на нём `https://github.com/basaltcode/neurogate` в `/opt/neurogate`. Поставь `uv`, выполни `uv sync`, скопируй конфиги из `.example`, помоги мне вписать `GROQ_API_KEY` в `.env`. Сгенерируй случайный `NEUROGATE_API_TOKEN`, поставь `NEUROGATE_HOST=0.0.0.0`. Создай systemd unit `neurogate.service`, открой 22 и 8765 в ufw, запусти. После запуска проверь `http://<IP>:8765/health` и покажи мне URL дашборда.
